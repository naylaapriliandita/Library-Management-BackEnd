...
async function main() {
    ...
    const adminUser = await prisma.user.findUnique({ where: { email: "admin@mail.com" } });
    const user1 = await prisma.user.findUnique({ where: { email: "user1@mail.com" } });
    const user2 = await prisma.user.findUnique({ where: { email: "user2@mail.com" } });
    const user3 = await prisma.user.findUnique({ where: { email: "user3@mail.com" } });
    
    // 3. AUTHOR SEEDING (Minimal 5 records)
    ...
    const jkRowling = await prisma.author.findUnique({ where: { name: "J.K. Rowling" } });
    const andreaHirata = await prisma.author.findUnique({ where: { name: "Andrea Hirata" } });
    const agathaChristie = await prisma.author.findUnique({ where: { name: "Agatha Christie" } });
    const harukiMurakami = await prisma.author.findUnique({ where: { name: "Haruki Murakami" } });
    const tereLiye = await prisma.author.findUnique({ where: { name: "Tere Liye" } });
    
    // 4. BOOK SEEDING (Minimal 5 records)
    ...
    const book1 = await prisma.book.findUnique({ where: { title: "Harry Potter 1" } }); 
    const book2 = await prisma.book.findUnique({ where: { title: "Laskar Pelangi" } }); 
    const book3 = await prisma.book.findUnique({ where: { title: "Norwegian Wood" } }); 
    const book4 = await prisma.book.findUnique({ where: { title: "And Then There Were None" } }); 
    
    // 5. TRANSACTION SEEDING (Minimal 5 records)
    const now = new Date();
    ...
    // 6. FINAL STOCK ADJUSTMENT: Kurangi stock untuk buku yang sedang BORROWED
    // Book1 dipinjam 2 kali, Book2 dipinjam 1 kali, Book3 dipinjam 0 kali (sudah dikembalikan), Book4 dipinjam 1 kali.
